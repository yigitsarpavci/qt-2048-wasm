// Core logic for the 2048 game mechanics, including grid management, movement, and history tracking.
...
// Encapsulates metadata for a tile transition, used for UI animations.
struct TileMove {
...
// Stores the full state of the game for undo operations.
struct GameSnapshot {
...
// Handles the deterministic game logic, move processing, and state management for 2048.
class GameEngine {
public:
    // Constructs the game engine with specified dimensions and goal.
    GameEngine(int n, int m, int k);

    // Processes a move attempt in the specified direction.
    bool move(Direction dir);

    // Resets the engine to a clean starting state with two initial tiles.
    void reset();

    // Reverts the game state to the most recent snapshot in the history stack.
    void undo();
    ...
    // Checks if the player has achieved the winning tile value.
    bool hasWon() const;
    
    // Determines if any valid moves remain (empty cells or possible merges).
    bool canMove() const;
    ...
    // Configures the probability of spawning a '2' vs a '4'.
