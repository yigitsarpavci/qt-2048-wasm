// Implementation of the 2048 main window, UI components, and event handling.
...
// Utility to generate consistent button styles for mode selection.
...
// Animates the tile's appearance with a scale-up effect.
...
// Constructs the UI tree and initializes style sheets.
void MainWindow::setupUI() {
    ...
    QString actionStyle = "QPushButton { background-color: #bbada0; color: #f9f6f2; font-weight: bold; border-radius: 4px; font-size: 16px; border: none; padding: 10px; } "
                          "QPushButton:hover { background-color: #a99e93; } "
                          "QPushButton:pressed { background-color: #8f7a66; }";
    ...
    toRestart->setFocusPolicy(Qt::NoFocus);
    ...
    auto changeMode = [this](GameMode newMode) {
...
// Triggers a visual feedback animation for score increases.
...
// Synchronizes the visual state with the underlying game engine.
...
void MainWindow::updateUI() {
    ...
    bool hasWinningTile = m_engine.hasWon();
    if (stuck) {
        m_normalBtn->setEnabled(false);
        m_unlimitedBtn->setEnabled(false);
        m_hardBtn->setEnabled(false);
    } else if (hasWinningTile) {
        m_normalBtn->setEnabled(false);
        m_unlimitedBtn->setEnabled(true);
        m_hardBtn->setEnabled(false);
    } else {
        m_normalBtn->setEnabled(true);
        m_unlimitedBtn->setEnabled(true);
...
void MainWindow::processMove(Direction dir) {
    ...
    if (m_engine.move(dir)) updateUI();
        ...
        if (m_shakeAnim) { m_shakeAnim->stop(); m_shakeAnim->deleteLater(); }
...
// Handles keyboard input for movement, undo, and mode selection.
...
// Reclaims keyboard focus on any mouse click within the window.
// Ensures gameplay keyboard input remains active after clicking empty areas.
...
// Manages the 5-second countdown for Hard Mode.
...
// Executes a random valid move in Hard Mode when the timer expires.
