// Main application window and user interface management for the 2048 game.
...
// Defines the available gameplay rulesets.
enum class GameMode {
...
// Represents the visual representation of a single game tile.
class TileWidget : public QWidget {
    Q_OBJECT
public:
    explicit TileWidget(QWidget *parent = nullptr);

    // Updates the tile's value and visual style.
    void setValue(int val);

    // Plays a pop-in entrance animation.
    void animatePopIn();
...
// The main interface class coordinating the game engine and UI components.
